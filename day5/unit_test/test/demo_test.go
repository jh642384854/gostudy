...
/**
	...
	3) TestAddUpper(t *tesing.T) 的形参类型必须是t *testing.T
	...
	8) 测试用例函数，并没有放在 main 函数中，也执行了，这就是测试用例的方便之处[原理图]. 9) PASS 表示测试用例运行成功，FAIL 表示测试用例运行失败
...
func TestGetSum(t *testing.T)  {
	res := unit_test.GetSum(5,3)

	if res != 9{
		t.Fatalf("GetSum() error %v",res)
	}
	t.Log("GetSum() success")
}